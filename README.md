...
功能开启时，浏览器标签页标题会显示「自动进入课堂」字样。如果没有显示，请检查打开的页面是否正确（对于荷塘雨课堂，页面 URL 应为 <https://pro.yuketang.cn/v2/web/index>）。

## 注意事项

1. 自动作答执行后会弹出通知提示。即使作答成功，网页端显示的作答状态也是未作答，但在网页端提交答案会提示题目已经作答过。可以打开题目列表检查边框是不是绿色的判断作答是否成功。
2. 如果习题列表中没有展示课件（即没有出现课件标题），可能是因为缓存问题导致脚本没有及时加载。此时可以尝试刷新页面解决该问题。
